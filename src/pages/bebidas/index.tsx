...
import { drinkAPI, getCategorylist as drinkCategories, drinkSmallAPI } from '../../services/getDrink';
import { foodAPI, getCategorylist as foodCategories, foodSmallAPi } from '../../services/getFood';
...
function Drinks() {
  ...
  useEffect(() => {
    ...
    const categoryAPI = async () => setCategories(await drinkCategories());
      ...
      <main>
        {respostaDrink
          .map(({ name, image, id }, index) => (
            <Link key={index} passHref href={`/bebidas/${id}`}>
